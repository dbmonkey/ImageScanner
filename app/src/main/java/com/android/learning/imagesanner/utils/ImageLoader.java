package com.android.learning.imagesanner.utils;
...
public class ImageLoader {
        ...
        if (mHandler == null) {
            mHandler = new Handler() {
                ...
                public void handleMessage(Message msg) {
                    ...
                    //获取图片，为imageview设置回调图片
                    ImgBeanHolder holder = (ImgBeanHolder) msg.obj;
                    ImageView imageView = holder.imageView;
                    Bitmap bm = holder.bitmap;
                    String path = holder.path;

                    //将path与getTag()的存储路径进行比较
    ...
    private ImageSize getImageViewWidth(ImageView imageView)
    {
        ...
        int height = params.height == LayoutParams.WRAP_CONTENT ? 0 : imageView
                .getHeight(); // 获取imageView实际高度
        if (height <= 0) {
            height = params.height; // 获取imageView在layout中声明的高度
        }
        if (height <= 0) {
            getImageViewFieldValue(imageView, "mMaxHeight");// 检查最大值
        }
        if (height <= 0) {
    ...
    private Bitmap decodeSampledBitmapFromResource(String pathName, int reqWidth, int reqHeight) {
        ...
        // 第一次解析将inJustDecodeBounds设置为true，来获取图片大小，并不把图片加载到内存中
        ...
        // 使用获取到的inSampleSize值再次解析图片，并把图片加载到内存中
